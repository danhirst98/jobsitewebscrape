...
def writejob(element, eid, cname, curl, cemail, jobtitle, jobdesc, jobcountry, jobstate, jobcity, jobzip="",
             jobdate=datetime.today().strftime('%Y-%m-%d'),
             jobexp=(datetime.today() + timedelta(days=30)).strftime('%Y-%m-%d'), active=1, approved=0, filled=0,
             feat=0, tags="", metas=""):
    ...
    if tags:
        ...
        for tg in tags:
            if len(tg) != 2:
                raise Exception(
                    'Wrong number of parameters for tags. Each tag must have 2 values in the list - 1 for type and 1 for title')
    ...
    if metas:
        ...
        for mt in metas:
            ...
            if len(mt) != 2:
                raise Exception(
                    'Wrong number of parameters for metas. Each meta must have 2 values in the list - 1 for name and 1 for value')
...
def writeXML(joblist, daysactive):
    ...
    for job in joblist:
        ...
        writejob(jobs, eid, cname, curl, cemail, jobtitle, jobdesc, jobcountry, jobstate, jobcity, jobzip, jobdate,
                 jobexp, active, approved, filled, feat, tags, metas)
...
dict = {'eid': "3", 'cname': "SpaceX", 'curl': "www.spacex.com", 'cemail': "sugma@spacex.com", 'jobtitle': "Chief Elon",
        'jobdesc': "youre going to be a huge Elon, cant wait", 'jobcountry': "USA", 'jobstate': "NC",
        'jobcity': "Chapel Hill", 'metas': [[0, 1], [1, 2]]}
